...
# 예제로 폭력성을 띄는 문장의 경우 Class 1을 부여하고 양호할 경우 Class 0을 부여한 후, 
# 각 문장에서 자주 등장하는 단어 중 어떤 단어가 영향을 주었는지 학습시킬 준비를 합니다.
def loadDataSet():
  ...
  classVec = [0,1,0,1,0,1]                            # 거친 문장일 경우 Class 1, 그렇지 않을 경우 Class 0를 부여합니다.
...
def setOfWords2Vec(vocabList, inputSet):
  ...
  for word in inputSet:                               # 입력된 문서의 단어들에 대하여  
    ...
    else: print "the word: %s is not in my Vocabulary!" % word    # 그렇지 않을 경우, 이 문장을 출력합니다.
...
# 앞서 사용된 setOfWords2Vec의 변형된 함수로써, 단어가 중복사용되어도 값이 매번 반영됩니다.
...
# 스팸메일을 필터링하는 함수를 정의합니다.
# training 및 test용 파일들의 경로는 아래와 같이 임의로 설정합니다.
def spamTest():
  docList=[]
  classList = []
  fullText =[]
  ...
  for docIndex in testSet:
    ...
    if classifyNB(array(wordVector),p0V,p1V,pSpam) != classList[docIndex]:
      errorCount += 1
  print 'the error rate is: ',float(errorCount)/len(testSet)                # 에러율을 계산합니다.


# 학습용 단어장 vocabList와 입력한 인자 문서 fulltext를 비교하여 자주 사용되는 단어를 추리는 함수를 정의합니다.
def calcMostFreq(vocabList, fullText):
  import operator                           # operator 모듈을 호출합니다.
  freqDict = {}                             # 딕셔너리를 만들기 위해 변수를 생성합니다.
  for token in vocabList:                   # vocabList에 있는 각 토큰에 대해서
    freqDict[token] = fullText.count(token) # 
  sortedFreq = sorted(freqDict.iteritems(), key=operator.itemgetter(1), reverse=True) # operator.itemgetter(1) 기준으로 정렬합니다.
  return sortedFreq[:30]                    # 상위 30개의 단어를 반환합니다.
...
def localWords(feed1,feed0):                                # 인자는 두 지역의 커뮤니티에서 불러온 데이터입니다.
  ...
  for i in range(minLen):                                   # 직전에 얻은 값 범위 안에서 for 문을 반복합니다.
    ...
    classList.append(1)                                     # classList에 1을 추가합니다. 여기서는 NY지역이 적용될 것입니다.
  ...
  trainingSet = range(2*minLen)                             # 학습시킬 데이터는 minLen의 2배 길이의 리스트로 생성하고,
  testSet=[]                                                # testSet은 빈 리스트로 시작합니다.
  for i in range(20):                                       # 0부터 19까지의 각 정수에 대해
    randIndex = int(random.uniform(0,len(trainingSet)))     
    testSet.append(trainingSet[randIndex])
    del(trainingSet[randIndex])
  trainMat=[]
  ...
  for docIndex in trainingSet:
    ...
    trainClasses.append(classList[docIndex])
  p0V,p1V,pSpam = trainNB0(array(trainMat),array(trainClasses))             # 학습용 함수 trainNBO의 반환된 세 개의 값을 변수로 설정합니다.
  ...
  for docIndex in testSet:                                                  # 테스트용 문서에 있는 각 인덱스에 대해서
    ...
    if classifyNB(array(wordVector),p0V,p1V,pSpam) != classList[docIndex]:  # classifyNB 함수를 한 번 더 거친 후의 반환값이
...
# 지역별로 자주 사용되는 단어를 예측하는 함수를 정의합니다.
def getTopWords(ny, sf):
  import operator                                                   # operator 모듈을 호출합니다.
  vocabList, p0V, p1V = localWords(ny, sf)                          # localWords 함수의 반환값을 받습니다.
  topNY = []                                                        # NY(뉴욕지역)용 리스트를 생성합니다.
  topSF = []                                                        # SF(샌프란시스코)용 리스트를 생성합니다.
  for i in range(len(p0V)):                                         # 
    if p0V[i] > -6.0: topSF.append((vocabList[i], p0V[i]))          # p0V의 i번째 항목의 자연로그값이 -6.0보다 크면 topSF에 추가합니다.
    if p1V[i] > -6.0: topNY.append((vocabList[i], p1V[i]))          # p1V의 i번째 항목의 자연로그값이 -6.0보다 크면 topNY에 추가합니다.
  sortedSF = sorted(topSF, key=lambda pair: pair[1], reverse=True)  # 만들어진 topSF를 pair[1]인 p0V[i] 기준으로 정렬합니다.
  print "***This is San Francisco***"                               # SF 지역에서 자주 사용되는 것으로 예측하는 단어들을
  for item in sortedSF:                                             # 출력합니다.
    print item[0]                                                   
  sortedNY = sorted(topNY, key=lambda pair:pair[1], reverse=True)   # 만들어진 topNY를 pair[1]인 p1V[i] 기준으로 정렬합니다.
  print "                      "                                    
  print "                      "                                    # 구분을 위해 세 줄의 공간을 만듭니다.
  print "                      "
  print "***This is New York***"                                    
  for item in sortedNY:                                             # NY 지역에서 자주 사용되는 것으로 예측하는 단어들을
    print item[0]                                                   # 출력합니다.
  return topNY, topSF                                               # *리스트 확인용으로 따로 반환값을 받았습니다.
